...
export function createWorkReport(blake2b: h.Blake2b, config: WorkReportConfig): WorkReport {
  if (config.results.length === 0) {
    throw new Error("WorkReport must contain at least one result");
  }

  if (config.results.length > 255) {
    throw new Error("WorkReport cannot contain more than 255 results (U8 limit)");
  }
