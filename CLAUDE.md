...
### Naming Rules
- Always write `jammin` in lowercase, even in headings, sentence starts, and product names
- Preserve casing for other product names (e.g., `typeberry`, `Polkajam`, `JAMNP`)

### Code Style
- **Linting & Formatting**: Use Biome (configured in `biome.json`)
  - Line width: 120 characters
  - Indent: 2 spaces
  - Line ending: LF
- **TypeScript**: Strict mode enabled with modern ESNext features
  - Module system: Preserve (bundler mode)
  - Use `.ts` file extensions in imports
- See `AI_GUIDELINES.md` for additional style expectations about prose and documentation
...
- `AI_GUIDELINES.md`: Required reading for naming conventions and style
