...
function getErrorHint(issue: ZodIssue): string | null {
  ...
  if (path.includes("deploy_with") && issue.code === "invalid_enum_value") {
...
export function handleError(error: unknown): never {
  if (error instanceof ConfigError) {
    // Check if cause is ZodError and format it nicely
